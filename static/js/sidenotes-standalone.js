...
(function () {
  ...
  const SidenotesStandalone = {
    ...
    sidenoteOfNumber: (number) => {
      return (
        SidenotesStandalone.sidenotes?.find(
          (sn) => SidenotesStandalone.noteNumber(sn) == number,
        ) ?? null
      );
    },
    ...
    citationOfNumber: (number) => {
      return (
        SidenotesStandalone.citations?.find(
          (cit) => SidenotesStandalone.noteNumber(cit) == number,
        ) ?? null
      );
    },
    ...
    updateSidenotePositionsIfNeeded: () => {
      ...
      if (SidenotesStandalone.positionUpdateQueued) return;
    ...
    updateSidenoteDispositions: () => {
      ...
      for (let [index, sidenote] of SidenotesStandalone.sidenotes.entries()) {
        ...
        sidenote.classList.toggle(
          'hidden',
          SidenotesStandalone.isWithinCollapsedBlock(
    ...
    updateSidenotePositions: () => {
      ...
      if (!SidenotesStandalone.mediaQueries.viewportWidthBreakpoint.matches) return;

      // Update dispositions
      SidenotesStandalone.updateSidenoteDispositions();

      // Get column bounding rects
      const leftColumnBoundingRect =
        SidenotesStandalone.sidenoteColumnLeft.getBoundingClientRect();
      const rightColumnBoundingRect =
        SidenotesStandalone.sidenoteColumnRight.getBoundingClientRect();
      ...
      document
        .querySelectorAll(config.potentiallyOverlappingElementsSelectors.join(', '))
        .forEach((element) => {
          if (SidenotesStandalone.isWithinCollapsedBlock(element)) return;
          ...
          if (
            ...
            proscribedVerticalRangesLeft.push({
              top:
                ...
                config.sidenoteSpacing -
                leftColumnBoundingRect.top,
              bottom:
      ...
      [proscribedVerticalRangesLeft, proscribedVerticalRangesRight].forEach(
        (ranges) => {
      ...
      if (config.useLeftColumn)
        columnSpecs.push([
          SidenotesStandalone.sidenoteColumnLeft,
          leftColumnBoundingRect,
          proscribedVerticalRangesLeft,
        ]);
      if (config.useRightColumn)
        columnSpecs.push([
          SidenotesStandalone.sidenoteColumnRight,
          rightColumnBoundingRect,
          proscribedVerticalRangesRight,
        ]);
      ...
      const defaultNotePosInCellForCitation = (cell, citation) => {
        return Math.max(
          0,
          Math.round(citation.getBoundingClientRect().top - cell.top + 4),
        );
      };
      ...
      for (let [index, citation] of SidenotesStandalone.citations.entries()) {
        ...
        const fittingLayoutCells = layoutCells.filter(
          (cell) => cell.room >= sidenote.offsetHeight,
        );
        ...
        const vDistanceToCell = (cell) => {
          if (
            citationBoundingRect.top > cell.top &&
        ...
        const overlapWithNote = (cell, note) => {
          ...
          const otherNotePosInCell = defaultNotePosInCellForCitation(
            cell,
            otherNoteCitation,
          );

          return otherNotePosInCell >
            notePosInCell + sidenote.offsetHeight + config.sidenoteSpacing ||
            notePosInCell >
              otherNotePosInCell + note.offsetHeight + config.sidenoteSpacing
            ? 0
            : Math.max(
                notePosInCell +
        ...
        const cellCrowdedness = (cell) => {
          return cell.sidenotes.reduce(
            (total, note) => total + overlapWithNote(cell, note),
            0,
          );
        };
        ...
        fittingLayoutCells.sort((cellA, cellB) => {
          return (
            vDistanceToCell(cellA) +
              cellCrowdedness(cellA) -
      ...
      const getDistance = (noteA, noteB) => {
        return (
          noteB.posInCell -
          (noteA.posInCell + noteA.offsetHeight + config.sidenoteSpacing)
        );
      };

      layoutCells.forEach((cell) => {
        if (cell.sidenotes.length == 0) return;
        ...
        cell.sidenotes.sort((noteA, noteB) => {
          return (
            noteA.posInCell - noteB.posInCell ||
        ...
        const pushNotesUp = (pushUpWhich, pushUpForce, bruteStrength = false) => {
          const roomToPush =
            pushUpWhich[0] == 0
              ...
              : Math.max(
                  0,
                  getDistance(
                    cell.sidenotes[pushUpWhich[0] - 1],
                    cell.sidenotes[pushUpWhich[0]],
                  ),
                );
          ...
          if (pushUpDistance <= roomToPush) {
            ...
            if (pushUpWhich[0] == 0) return pushUpForce - roomToPush;
        ...
        for (let i = 1; i < cell.sidenotes.length; i++) {
          ...
          thisNote.posInCell +=
            overlapAbove - pushUpForce + pushNotesUp([i - 1], pushUpForce);
        }

        // Check bottom overlap
        const overlapOfBottom = Math.max(
          0,
          cell.sidenotes[cell.sidenotes.length - 1].posInCell +
            cell.sidenotes[cell.sidenotes.length - 1].offsetHeight -
            cell.height,
        );
        ...
        cell.sidenotes.forEach((sidenote) => {
          sidenote.style.top =
            Math.round(sidenote.posInCell) + (cell.top - cell.columnRect.top) + 'px';
        });
      });
    ...
    constructSidenotes: () => {
      ...
      if (
        ...
        SidenotesStandalone.sidenoteColumnRight = SidenotesStandalone.newElement(
          'DIV',
          {
      ...
      const allCitations = Array.from(
        document.querySelectorAll(config.citationSelector),
      );
      const newCitations = allCitations.filter((citation) => {
        return (
          SidenotesStandalone.citationOfNumber(
      ...
      SidenotesStandalone.citations.sort(
        (a, b) => SidenotesStandalone.noteNumber(a) - SidenotesStandalone.noteNumber(b),
      );
      ...
      SidenotesStandalone.citations.forEach((citation) => {
        ...
        // Create self-link
        const selfLink = SidenotesStandalone.newElement(
          'A',
          {
        ...
        citation.addEventListener('click', (event) => {
          ...
          if (SidenotesStandalone.mediaQueries.viewportWidthBreakpoint.matches) {
            ...
            const sidenoteRect = sidenote.getBoundingClientRect();
            const scrollOffset =
              window.scrollY + sidenoteRect.top - sidenotePadding - 10;
      ...
      requestAnimationFrame(() => {
        ...
        // Fade and slide sidenotes in with a stagger
    ...
    updateMarginNoteStyle: () => {
      ...
      document.querySelectorAll('.marginnote').forEach((marginNote) => {
        const inline =
          marginNote.closest(config.constrainMarginNotesWithinSelectors.join(', ')) ||
    ...
    setup: () => {
      ...
      SidenotesStandalone.mediaQueries.viewportWidthBreakpoint = matchMedia(
        `(min-width: ${config.minimumViewportWidthForSidenotes}px)`,
      );
      SidenotesStandalone.mediaQueries.marginNoteViewportWidthBreakpoint = matchMedia(
        `(min-width: ${config.minimumViewportWidthForSidenoteMarginNotes}px)`,
      );
      ...
      SidenotesStandalone.mediaQueries.viewportWidthBreakpoint.addListener(
        handleSidenoteViewportChange,
      );
      ...
      SidenotesStandalone.mediaQueries.marginNoteViewportWidthBreakpoint.addListener(
        handleMarginNoteViewportChange,
      );
