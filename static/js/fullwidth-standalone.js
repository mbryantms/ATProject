  ...
  const FullWidthStandalone = {
    ...
    updateLayoutMetrics: () => {
      ...
      const container = document.querySelector(
        FullWidthStandalone.config.containerSelector,
      );
      ...
      const rightMargin =
        FullWidthStandalone.layoutState.pageWidth - containerRect.right;
    ...
    applyFullWidthMargins: (element) => {
      ...
      const additionalLeftAdjustment =
        FullWidthStandalone.calculateAdditionalLeftAdjustment(element);
      ...
      FullWidthStandalone.log(
        `Applied margins to ${element.tagName} with additional adjustment: ${additionalLeftAdjustment}px`,
      );
    },
    ...
    processFullWidthElements: () => {
      ...
      const fullWidthElements = document.querySelectorAll(
        FullWidthStandalone.config.fullWidthSelector,
      );
          ...
          mutations.forEach((mutation) => {
            mutation.addedNodes.forEach((node) => {
              if (node.nodeType === Node.ELEMENT_NODE) {
                ...
                if (
                  node.matches &&
                  node.matches(FullWidthStandalone.config.fullWidthSelector)
                ) {
                  shouldReprocess = true;
                } else if (
                  node.querySelector &&
                  node.querySelector(FullWidthStandalone.config.fullWidthSelector)
                ) {
                  shouldReprocess = true;
                }
              }
