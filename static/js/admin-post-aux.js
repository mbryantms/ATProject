...
(function(){if(window.__mkCsHelpBound)return;window.__mkCsHelpBound=true;document.addEventListener('click',function(e){if(e.target&&e.target.classList.contains('mk-citestyle-help-btn')){e.preventDefault();var p=e.target.nextElementSibling;var shown=p.style.display==='block';document.querySelectorAll('.mk-citestyle-help-panel').forEach(function(el){el.style.display='none';});if(!shown){p.style.display='block';var r=e.target.getBoundingClientRect();p.style.left=(window.scrollX+r.left)+'px';p.style.top=(window.scrollY+r.bottom+6)+'px';}}else if(!(e.target.closest&&e.target.closest('.mk-citestyle-help-panel'))){document.querySelectorAll('.mk-citestyle-help-panel').forEach(function(el){el.style.display='none';});}});})();
...
// --- Copy buttons ([data-clipboard-text]) and asset cards (.mk-asset-card[data-ref]) ---
// Delegated so no inline onclick handlers are needed (the site's nonce CSP
// blocks inline handlers). Replaces the old per-element onclick / setTimeout
// binding scripts.
(function () {
