...
from engine.links.extractor import get_link_statistics, update_post_links
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if post_id:
            ...
            if not posts.exists():
                self.stdout.write(self.style.ERROR(f"No post found with ID: {post_id}"))
                return
        elif post_slug:
        ...
        if total_stats["links_failed"] > 0:
            self.stdout.write(
                self.style.WARNING(f"Links failed:      {total_stats['links_failed']}")
            )
            if verbose:
                self.stdout.write(
                    self.style.WARNING(
        ...
        if show_stats and not dry_run:
            ...
            try:
                ...
                self.stdout.write(
                    f"Total links:              {link_stats['total_links']}"
                )
                self.stdout.write(
                    f"Total posts:              {link_stats['total_posts']}"
                )
                self.stdout.write(
