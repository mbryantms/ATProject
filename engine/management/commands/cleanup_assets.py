...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if delete_files and not dry_run:
            self.stdout.write(
                self.style.WARNING(
                    "  WARNING: Files will be permanently deleted from R2!\n"
                )
            )
        ...
        if examples:
            ...
            for r in examples:
                try:
                    ...
                    status = (
                        "(deleted)" if r.asset and r.asset.is_deleted else "(missing)"
                    )
    ...
    def _cleanup_unused_assets(self, dry_run, soft_deleted, days, delete_files):
        ...
        if examples:
            ...
            for asset in examples:
                ...
                self.stdout.write(f"    - {asset.key} ({asset.asset_type}) {status}")
