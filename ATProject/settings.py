...
# ==============================================================================
# Fail fast on misconfigurations that are silently dangerous in production.
# Skipped under DEBUG, tests, and the Docker `collectstatic` build step (which
# runs with placeholder secrets and no real broker — see the Dockerfile). The
# guards therefore fire for the runtime processes that matter: `migrate` and
# Gunicorn/Celery.
if not DEBUG and not TESTING and "collectstatic" not in sys.argv:
    if len(SECRET_KEY) < 50 or SECRET_KEY.startswith("django-insecure-"):
        raise environ.ImproperlyConfigured(
            "SECRET_KEY is too short or looks autogenerated. Set a long, random "
            "SECRET_KEY (at least 50 characters) in the production environment."
        )
    if not env("REDIS_URL", default=None):
        raise environ.ImproperlyConfigured(
            "REDIS_URL is not set. Without it the Celery broker silently falls "
            "back to an in-memory transport that drops every task — posts would "
            "never re-render and scheduled publishing would never run. Set "
            "REDIS_URL in the production environment."
        )
